document.addEventListener('DOMContentLoaded', () => {
    ...
    if(!mine.length){
      tbody.innerHTML = `
        <tr>
          <td colspan="6">
            <div class="empty">
              <div class="eyebrow">Sin productos</div>
              Publica tu primer juego en Elite Games.
            </div>
          </td>
        </tr>`;
      return;
  ...
  tbody.addEventListener('click', e => {
    ...
    if(editId){
      openModal(egGetProducts().find(p => p.id == editId));
    }

    if(delId){
      if(confirm('¿Eliminar este producto de tu catálogo?')){
        egSetProducts(
          egGetProducts().filter(
            p => !(p.id == delId && p.proveedor === empresa)
          )
        );
