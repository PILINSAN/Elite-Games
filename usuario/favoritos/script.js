document.addEventListener('DOMContentLoaded', () => {
  ...
  function render() {
    ...
    if (!products.length) {
      grid.innerHTML = `
        <div class="empty" style="grid-column:1/-1;">
          <div class="eyebrow">Sin favoritos aún</div>
          Marca juegos con el corazón desde el catálogo.
        </div>`;
      return;
    }
    ...
    grid.innerHTML = products.map(p => `
      <div class="card game-card">
        <div class="thumb">
          <img src="${p.img}" alt="${p.nombre}" onerror="this.parentElement.innerHTML='🎮'">
        </div>
        ...
        <div class="row">
          ...
          <button class="fav-btn active" data-remove="${p.id}">
            ❤ Quitar
          </button>
        </div>
      </div>
  ...
  grid.addEventListener('click', (e) => {
    ...
    if (removeId) {
      let favs = egGetFav(session.username) || [];
      favs = favs.filter(id => id != removeId);

      egSetFav(session.username, favs);
